<!doctype html>
<html class="no-js" lang="en">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <meta http-equiv="x-ua-compatible" content="ie=edge">
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8">
        
        <meta name="title" content="Buy Food">
        <meta name="description" content="You can search food from your room">
        <meta name="keywords" content="food,checken">
        <meta name="robots" content="index, follow">
        <meta name="language" content="English">
        <meta name="revisit-after" content="10 days">
        <meta name="author" content="ravijeyaraman">
        <!-- Favico Icon -->
		<link rel="shortcut icon" type="image/x-icon" href="./assets/img/logo.webp">
        
        <title>CATERING</title>
		<!-- CSS here -->
        <link rel="stylesheet" href="assets/css/all.css"> 
    </head>
    <body>

       
    <!-- header-area START -->
    <div class="header-area">
        <div class="header__top">
            <div class="container">
                <div class="row align-items-center">
                    <div class="col-lg-6 col-md-6">
                        <div class="headerTop__left">
                            <p>chapel hill, NC (984-234-3409)  <span>|</span> <a href="mailto:info@LNLrestaurant.com"><i class="fal fa-envelope"></i> info@LNLrestaurant.com</a></p>
                        </div>
                    </div>
                    <div class="col-lg-6 col-md-6">
                        <div class="headerTop__right">
                            <ul>
                                <li><a href="#"><i class="fas fa-clock"></i> RESERVATIONS</a></li>
                                <li><a href="#"><i class="fas fa-gift"></i> GIFT CARDS</a></li>
                                <li><a href=""><img src="assets/img/facebookImg.webp" alt=""></a></li>
                                <li><a href=""><img src="assets/img/InstagramImg.webp" alt=""></a></li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="container lg">
            <div class="row align-items-center">
                <div class="col-lg-5 d-none d-lg-block text-right">
                    <div class="menu-area">
                        <nav class="left">
                            <ul id="nav" >
                                <li><a href="index.jsp">Home</a></li>
                                <li><a href="menu.jsp">MENU</a></li> 
                                <li><a href="bar.jsp" >BAR</a></li> 
                                <li><a href="CATERING.jsp" class="active">CATERING</a></li>  
                            </ul>
                        </nav>
                    </div>
                </div>
                <div class="col-lg-2 col-6 ">
                    <div class="logo-area">
                        <a href="index.jsp"><img src="assets/img/logo.webp" alt=""></a>
                    </div>
                </div>
                <div class="col-lg-5 d-none d-lg-block text-right">
                    <div class="menu-area">
                        <nav class="right">
                            <ul id="nav" class="justify-content-start">
                                <li><a href="gallery.jsp">GALLERY</a></li>
                                <li><a href="RESERVE.jsp" class="greenBtn">RESERVE</a></li> 
                                <li><a href="#" class="ylwBtn">ORDER</a></li> 
                            </ul>
                        </nav>
                    </div>
                </div>
                <div class="col-6 d-block d-lg-none text-center">
                    <div class="bar">
                        <a class="bar-icon siteBar-btn" href="#">
                            <span></span>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </div>
        <!-- mobile-menu START -->
        <div class="mobile-menu">
            <a href="#" class="bars siteBar-btn"><svg class="bi bi-x" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" d="M11.854 4.146a.5.5 0 010 .708l-7 7a.5.5 0 01-.708-.708l7-7a.5.5 0 01.708 0z" clip-rule="evenodd"/>
                <path fill-rule="evenodd" d="M4.146 4.146a.5.5 0 000 .708l7 7a.5.5 0 00.708-.708l-7-7a.5.5 0 00-.708 0z" clip-rule="evenodd"/>
            </svg></a> 
            <nav> 
                <ul>
                    <li><a href="index.jsp" >Home</a></li>
                    <li><a href="menu.jsp">MENU</a></li> 
                    <li><a href="bar.jsp">BAR</a></li> 
                    <li><a href="CATERING.jsp" class="active">CATERING</a></li>   
                    <li><a href="gallery.jsp">GALLERY</a></li>
                    <li><a href="RESERVE.jsp" class="greenBtn">RESERVE</a></li> 
                    <li><a href="#" class="ylwBtn">ORDER</a></li> 
                </ul>
            </nav> 

            <div class="mobile__btm">
                <div class="headerTop__right">
                    <ul>
                        <li><a href="#"><i class="fas fa-clock"></i> RESERVATIONS</a></li>
                        <li><a href="#"><i class="fas fa-gift"></i> GIFT CARDS</a></li>
                        <li><a href=""><i class="fab fa-facebook-square"></i></a></li>
                        <li><a href=""><i class="fab fa-instagram-square"></i></a></li>
                    </ul>
                </div>
                
                <div class="headerTop__left">
                    <p>chapel hill, NC (984-234-3409)  <hr> <a href="mailto:info@LNLrestaurant.com"><i class="fal fa-envelope"></i> info@LNLrestaurant.com</a></p>
                </div>
            </div>
        </div> 
    <!-- header-area END -->


    <main>

         <!---------banner Section START ---------->
        <section class="banner_area " data-aos="fade-up"  style="background-image: url(./assets/img/CATERING-bg.webp);">
            <div class="container">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="banner_title">
                            <h2>CATERING</h2>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!---------banner Section End ---------->

        
        <!---------contact Section START ---------->
        <section class="contact-area sec__padding">
            <div class="container">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="contact__text mb-5" data-aos="fade-up" >
                            <p>At Lime & Lemon Indian Grill & Bar we always custom create menus to ensure our clients receive innovative, fresh, unique cuisine and top-notch service. We can individually tailor menus to meet the needs of your special event. To receive an estimate for services, or begin planning your event, please submit the following form. Have an idea? Don't be afraid to ask!</p>
                        </div>
                    </div>
                </div>
                <div class="form__wrp">
                    <form action="">
                        <div class="row">
                            <div class="col-lg-6 col-md-6" >
                                <div class="input__blk">
                                    <label for="">Name * </label>
                                    <input type="text" placeholder="">
                                </div>
                            </div>
                            <div class="col-lg-6 col-md-6" >
                                <div class="input__blk">
                                    <label for="">Contact Number *
                                    </label>
                                    <input type="text" placeholder="">
                                </div>
                            </div>
                            <div class="col-lg-6 col-md-6" >
                                <div class="input__blk">
                                    <label for="">Contact Email</label>
                                    <input type="text" placeholder="">
                                </div>
                            </div>
                            <div class="col-lg-6 col-md-6" >
                                <div class="select__blk">
                                    <label for="">Type of Event </label>
                                    <select class="wide">
                                        <option data-display="Please select the event type">Nothing</option>
                                        <option value="1">Some option</option>
                                        <option value="2">Another option</option>
                                        <option value="3" disabled>A disabled option</option>
                                        <option value="4">Potato</option>
                                      </select>
                                </div>
                            </div>
                            <div class="col-lg-3 col-md-6" >
                                <div class="input__blk">
                                    <label for="">Date of Event</label>
                                    <input type="date" placeholder="">
                                </div>
                            </div>
                            <div class="col-lg-3 col-md-6" >
                                <div class="input__blk">
                                    <label for="">Time of Event</label>
                                    <input type="time" placeholder="">
                                </div>
                            </div>
                            <div class="col-lg-6 "  >
                                <div class="input__blk">
                                    <label for=""># of Guests</label>
                                    <input type="text" placeholder="">
                                </div>
                            </div>
                            <div class="col-lg-12"  >
                                <div class="input__blk">
                                    <label for="">Description </label>
                                    <textarea name="" id="" ></textarea>
                                </div>
                            </div>
                            <div class="col-lg-12"  >
                                <div class="contact__text">
                                    <p>The information you provide will help us serve your catering needs more efficiently. Once your request is submitted, our event coordinators will contact you to go over details further. PLEASE NOTE: This is not an order form. Quotes will be based on menu and service requests. Credit card fees may apply.</p>
                                    <p>Please enter the Captcha below
                                    </p>
                                </div>
                                <div class="Captcha__box">
                                    <span><del>evs1s3pjt8n</del></span>
                                    <div class="input__blk">
                                        <input type="text">
                                    </div>
                                </div>
                            </div>
                            <div class=""  >
                                <button type="submit" class="form__btn">Submit</button>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        </section>
        <!---------contact Section End ---------->


    </main>

        <!---------footer Section START ---------->
        <footer class="footer-area">
            <div class="container">
                <div class="row">
                    <div class="col-lg-3 col-md-6">
                        <div class="footer__nav">
                            <h5>CONTACT US  </h5>
                            <a href="#"><i class="fas fa-map-marker-alt"></i> Chapel Hill</a>
                            <p>
                                100 Meadowmont Village Cir #101, <br>
                                Chapel Hill, NC 27517 <br>
                                984-234-3409 <br>
                                info@LNLrestaurant.com
                            </p>
                        </div>
                    </div>
                    <div class="col-lg-5 col-md-6">
                        <div class="footer__nav">
                            <h5>HOURS</h5>
                            <a href="#"><i class="fas fa-map-marker-alt"></i> Chapel Hill</a>
                            <p>
                                Mon : 11:00AM – 2:00PM & 5:00PM – 9:30PM <br>
                                Tue : 11:00AM – 2:00PM & 5:00PM – 9:30PM <br>
                                Wed : 11:00AM – 2:00PM & 5:00PM – 9:30PM <br>
                                Thu : 11:00AM – 2:00PM & 5:00PM – 9:30PM <br>
                                Fri : 11:00AM – 2:00PM & 5:00PM – 10:00PM <br>
                                Sat : 11:30AM – 2:30PM & 5:00PM – 10:00PM <br>
                                Sun : 11:30AM – 2:30PM & 5:00PM – 9:00PM
                            </p>
                        </div>
                    </div>
                    <div class="col-lg-4">
                        <div class="footer__nav fNav__last">
                            <h5>OUR LOCATIONS  </h5>
                            <a href="#"> Chapel Hill</a>
                            <a href="#">  DURHAM</a>
                            <a href="#">  JACKSONVILLE</a>
                            <a href="#">  RALEIGH</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="footer__btm">
                <p>Copyright 2020 Lime & Lemon - All Rights Reserved.</p>
            </div>
        </footer>
        <!---------footer Section End ---------->

    <!-- JS here --> 
    <script src="assets/js/jquery-3.4.1.min.js"></script>
    <script src="assets/js/popper.min.js"></script>
    <script src="assets/js/bootstrap.min.js"></script> 
    <script src="assets/js/owl.carousel.min.js"></script> 
    <script src="assets/js/jquery.nice-select.min.js"></script> 
    <script src="assets/js/main.js"></script>
</body>
</html>


